'use strict';
    ...
    let vm = new Vue({
      ...
      methods: {
        ...
        hit() {
          if(!this.playing) {
            this.msg("Click <strong>Deal</strong> para reiniciar.");
            return;
          }
          if( this.playerScore <= 21 )
            this.player.push( this.deck.pop() );
          if(this.playerScore > 21)
            this.msg("<strong>Jugador</strong> ha perdido", 'warning');
        },
        stand() {
          if(!this.playing) {
            this.msg("Click <strong>Deal</strong> para reiniciar.");
          ...
          if(playerWin) {
            this.score++;
            this.msg('<strong>Jugador</strong> gana.', 'success');
          } else {
            this.score--;
            this.msg('<strong>AMX</strong> gana.', 'danger');
          }
        },
        deal() {
          this.newDeck();
          ...
          if(this.playing) {
            this.score--;
            this.msg("<strong>Jugador</strong> lose. <strong>Pedir Carta</strong> o <strong>Mantenerse</strong>?", "success");
          } else {
            this.playing = true;
            this.msg("<strong>Pedir Carta</strong> o <strong>Mantenerse</strong>?");
          }
        },
        calcScore(hands) {
          let score = 0;
